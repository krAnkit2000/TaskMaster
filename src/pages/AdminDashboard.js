...
import {
  ...
  orderBy, updateDoc, doc, setDoc, deleteDoc, where
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(qArchive, (snapshot) => {
      ...
      // 2. Pending tasks - SIRF is admin ke createdBy wale
  ...
  // === FETCH EMPLOYEES (ONLY THIS ADMIN'S CREATED USERS) ===
  useEffect(() => {
  ...
  const assignTask = async (e) => {
    ...
    try {
      const taskData = {
        ...
        status: 'Pending',
        assignedAt: serverTimestamp(),
        createdBy: currentAdminUid, // ✅ Kis admin ne assign kiya
      };
  ...
  // === EMPLOYEE MANAGEMENT LOGIC ===
  // ✅ FIX: Secondary app use karte hain taaki naya user create karne se
  // current admin ka session logout NA ho (Firebase ka known behavior hai)
  const createEmployee = async (e) => {
    e.preventDefault();
    setLoading(true);

    // Secondary Firebase app banayein (temporary, sirf is operation ke liye)
  ...
  // === STATS & SEARCH (already scoped since tasksList/employeesList are filtered) ===
  const totalTasks = tasksList.length;
  const completedTasks = tasksList.filter(t => t.status === 'Completed').length;
  const pendingTasks = tasksList.filter(t => t.status === 'Pending').length;
